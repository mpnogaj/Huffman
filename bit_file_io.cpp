...
bit_file_io::bit_file_io(std::fstream &file_stream, size_t read_buff_size, size_t write_buff_size):
	file_stream_(file_stream), r_buff_size_(std::min(read_buff_size, static_cast<size_t>(1))),
	w_buff_size_(std::min(read_buff_size, static_cast<size_t>(1)))
{
...
/**
 * \brief Causes w_bit_buf_ to be saved to w_buff_. If w_buff_ achieves max size it's being flushed
 */
...
/**
 * \brief Causes w_buff_ to be written to file. Any data in w_bit_buf_ is ignored
 */
void bit_file_io::flush_buffer()
{
	...
	file_stream_.write(reinterpret_cast<char *>(this->w_buff_), sizeof(uint8_t) * this->w_buff_cnt_);
	this->w_buff_cnt_ = 0;
}
...
bool bit_file_io::read_bit(uint8_t &bit)
{
	if (this->r_bit_buf_size_ == 0)
	{
		...
		{
			if (!this->file_stream_.read(reinterpret_cast<char*>(this->r_buff_), this->r_buff_size_))
				return false;
